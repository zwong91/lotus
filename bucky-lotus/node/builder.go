...
func ProofParams(maddr address.Address, fnapi v1api.FullNode) Option {
	proof, _ := modules.SealProofType(dtypes.MinerAddress(maddr), fnapi)

	return func(settings *Settings) error {
		return Options(
			Override(new(dtypes.MinerAddress), dtypes.MinerAddress(maddr)),
			Override(new(dtypes.MinerID), modules.MinerID),
			Override(new(abi.RegisteredSealProof), proof),
		)(settings)
	}
}
