...
func (s *WindowPoStScheduler) runPoStCycle(ctx context.Context, di dline.Info, ts *types.TipSet) ([]miner.SubmitWindowedPoStParams, error) {
	...
	// Get the partitions for the given deadline  ep:  ./lotus-miner proving deadline 44
	...
	for batchIdx, batch := range partitionBatches {
		...
		for retries := 0; ; retries++ {
			...
			for partIdx, partition := range batch {
				// TODO: Can do this in parallel,  parallel task to workers modified by <wangzhi@buckyos.com>
				if raw, ok := os.LookupEnv("LOTUS_WPOST_TASK"); ok {
					// multi worker wdpost parallel each worker do plan job
					has := false
					...
					if err := json.Unmarshal([]byte(raw), &tasks); err != nil {
						...
						for _, v := range tasks.taskIdx {
							if len(v) != 3 {
								log.Errorf("failed to format LOTUS_WPOST_TASK: %w, expected: 3 elem (dlIdx, batchIdx, partIdx)", v)
							} else {
								if di.Index == uint64(v[0]) && batchPartitionStartIdx == v[1] && partIdx == v[2] {
									has = true
									break
								}
							}
					...
					// 没有匹配的partition task 就continue, 直到匹配才走下面的逻辑
					if !has {
						continue
					}
				}
	...
	if _, ok := os.LookupEnv("LOTUS_WPOST_TASK"); !ok {
		os.Unsetenv("LOTUS_WPOST_TASK")
	}
