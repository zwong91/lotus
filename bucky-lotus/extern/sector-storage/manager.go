...
func New(ctx context.Context, nodeApi v1api.FullNode, lstor *stores.Local, stor *stores.Remote, ls stores.LocalStorage, si stores.SectorIndex, sc SealerConfig, wss WorkerStateStore, mss ManagerStateStore) (*Manager, error) {
	...
	if _, ok := os.LookupEnv("LOTUS_WDSCHEDULE"); ok {
		go func() {
			if err := m.schedWPoSt(ctx); err != nil {
				log.Errorf("schedWPoSt failed: %s", err)
			}
		}()
	}
	...
	if _, ok := os.LookupEnv("LOTUS_WDPOSTONLY"); ok {
		localTasks = []sealtasks.TaskType{}
	}
...
func (m *Manager) batchPartitions(partitions []api.Partition, nv network.Version, p abi.RegisteredPoStProof) ([][]api.Partition, error) {
	...
	//                              <1><2> (3rd doesn't fit)
	partitionsPerMsg, err := policy.GetMaxPoStPartitions(nv, p)
	if err != nil {
...
func (m *Manager) schedWPoSt(ctx context.Context) error {
	var (
	...
	if env, ok := os.LookupEnv("LOTUS_MADDR"); ok {
		v, err := address.NewFromString(env)
		if err != nil {
			return err
		}
		maddr = v
	} else {
		log.Errorf("This miner address not found.")
		return xerrors.Errorf("This miner address not found.")
	}
LOOP:
	// not fine to panic after this point
	for {
		if notifs == nil {
			notifs, err = m.nodeApi.ChainNotify(ctx)
			if err != nil {
		...
		case changes, ok := <-notifs:
			...
			if !gotCur {
				...
				m.update(ctx, maddr, nil, chg.Val)
			...
			m.update(ctx, maddr, lowest, highest)
...
func (m *Manager) update(ctx context.Context, maddr address.Address, revert, apply *types.TipSet) {
	if apply == nil {
		log.Error("no new tipset in window post Manager.update")
		return
	}

	di, err := m.nodeApi.StateMinerProvingDeadline(ctx, maddr, apply.Key())
	if err != nil {
		log.Errorf("StateMinerProvingDeadline error: %+v", err)
	}
	if !di.PeriodStarted() {
		log.Debugf("not proving anything yet")
		return
	}

	// Get the partitions for the given deadline  ep:  ./lotus-miner proving deadline 0/47
	partitions, err := m.nodeApi.StateMinerPartitions(ctx, maddr, di.Index, apply.Key())
	if err != nil {
		log.Errorf("getting partitions: %w", err)
		return
	}

	nv, err := m.nodeApi.StateNetworkVersion(ctx, apply.Key())
	if err != nil {
		log.Errorf("getting network version: %w", err)
		return
	}

	mi, err := m.nodeApi.StateMinerInfo(ctx, maddr, types.EmptyTSK)
	if err != nil {
		log.Errorf("getting sector size: %w", err)
	}

	// Split partitions into batches, so as not to exceed the number of sectors
	// allowed in a single message
	partitionBatches, err := m.batchPartitions(partitions, nv, mi.WindowPoStProofType)
	if err != nil {
		log.Errorf("batchPartitions error: %+v", err)
		return
	}
	sector := storage.SectorRef{
		ID:        abi.SectorID{Miner: 1000, Number: 1<<64 - 1},
	...
	for batchIdx, batch := range partitionBatches {
		...
		for partIdx, partition := range batch {
			log.Infof("partitions proving info: %w, %w", batchPartitionStartIdx, partIdx, partition)
			// pending dlIdx, partIdx, partition
			m.assignWPoSt(ctx, sector, int(di.Index), batchPartitionStartIdx, partIdx)
		}
	}

}

func (m *Manager) assignWPoSt(ctx context.Context, sector storage.SectorRef, dlIdx, batchIdx, partIdx int) error {
	var selector WorkerSelector
	var err error
	selector = newAllocSelector(m.index, storiface.FTNone, storiface.PathSealing)

	err = m.sched.Schedule(ctx, sector, sealtasks.TTWPoSt, selector, schedNop, func(ctx context.Context, w Worker) error {
		p, err := m.waitSimpleCall(ctx)(w.DispatchWPoSt(ctx, sector, dlIdx, batchIdx, partIdx))
		if err != nil {
			log.Errorf("assignWPoSt err: %w", p)
			return err
		}
		if p != nil {
			log.Errorf("assignWPoSt p err: %w", p)
		}
		return nil
	})
...
func (m *Manager) ReturnDispatchWPoSt(ctx context.Context, callID storiface.CallID, err *storiface.CallError) error {
	return m.returnResult(ctx, callID, nil, err)
}
