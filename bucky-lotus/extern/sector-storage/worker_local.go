...
func newLocalWorker(executor ExecutorFunc, wcfg WorkerConfig, store stores.Store, local *stores.Local, sindex stores.SectorIndex, ret storiface.WorkerReturn, cst *statestore.StateStore) *LocalWorker {
	...
	paths, err := local.Local(context.TODO())
	if err != nil {
		log.Errorf("getting local storage config: %w", err)
	}
	for _, path := range paths {
		local_repo = append(local_repo, path.LocalPath)
	}

	perfCli, err := tracing.NewPerfClient(local_repo)
	if err != nil {
		log.Errorf("new perf client error: %+v", err)
	}
...
var returnFunc = map[ReturnType]func(context.Context, storiface.CallID, storiface.WorkerReturn, interface{}, *storiface.CallError) error{
	...
	WPoSt:           rfunc(storiface.WorkerReturn.ReturnDispatchWPoSt),
}
...
func (l *LocalWorker) asyncCall(ctx context.Context, sector storage.SectorRef, rt ReturnType, work func(ctx context.Context, ci storiface.CallID) (interface{}, error)) (storiface.CallID, error) {
	...
	start := time.Now()
	if _, err := l.perfClient.WorkerTask(sector.ID.Miner.String(), sector.ID.Number.String(), string(rt), "pending", 0, time.Now().Unix()); err != nil {
		log.Errorf("tracing call (pending): %+v", err)
	}
	...
	go func() {
		...
		if err != nil {
			...
			if err != nil {
				...
				if _, err := l.perfClient.WorkerTask(sector.ID.Miner.String(), sector.ID.Number.String(), string(rt), err.Error(), time.Since(start).Milliseconds(), time.Now().Unix()); err != nil {
					log.Errorf("tracing call (marshaling results): %+v", err)
				}
			} else {
				if err := l.ct.onDone(ci, rb); err != nil {
					log.Errorf("tracking call (done): %+v", err)
				}
				if _, err := l.perfClient.WorkerTask(sector.ID.Miner.String(), sector.ID.Number.String(), string(rt), "processing", time.Since(start).Milliseconds(), time.Now().Unix()); err != nil {
					log.Errorf("tracing call (processing): %+v", err)
				}
			}
		}
		...
		if doReturn(ctx, rt, ci, l.ret, res, toCallError(err)) {
			...
			if _, err := l.perfClient.WorkerTask(sector.ID.Miner.String(), sector.ID.Number.String(), string(rt), "solved", time.Since(start).Milliseconds(), time.Now().Unix()); err != nil {
				log.Errorf("tracing call (solved): %+v", err)
			}
		}
	}()
...
func (l *LocalWorker) DispatchWPoSt(ctx context.Context, sector storage.SectorRef, dlIndx, batchIdx, partIdx int) (storiface.CallID, error) {
	return l.asyncCall(ctx, sector, WPoSt, func(ctx context.Context, ci storiface.CallID) (interface{}, error) {
		type wpostTask struct {
			TaskIdx map[string][]int
		}

		split := "+"
		var build strings.Builder
		build.WriteString(strconv.Itoa(dlIndx))
		build.WriteString(split)
		build.WriteString(strconv.Itoa(batchIdx))
		build.WriteString(split)
		build.WriteString(strconv.Itoa(partIdx))
		key := build.String()

		taskMap := make(map[string][]int)

		value := []int{dlIndx, batchIdx, partIdx}
		taskMap[key] = value
		in := wpostTask{
			TaskIdx: taskMap,
		}
		...
		if err != nil {
			log.Errorf("failed to marshal in wdpost: %w", err)
			return ci, err
		}

		if raw, ok := os.LookupEnv("LOTUS_WPOST_TASK"); ok {
			task := wpostTask{TaskIdx: make(map[string][]int)}
			if err := json.Unmarshal([]byte(raw), &task); err != nil {
				log.Errorf("failed to unmarshal LOTUS_WPOST_TASK: %w", err)
			} else {
				if _, isOk := task.TaskIdx[key]; isOk {
					return ci, nil
				} else {
					task.TaskIdx[key] = value
					v, err := json.Marshal(task)
					if err != nil {
						log.Errorf("failed to marshal task wdpost: %w", err)
						return ci, err
					}
					os.Setenv("LOTUS_WPOST_TASK", string(v))
				}
			}
		} else {
			os.Setenv("LOTUS_WPOST_TASK", string(data))
		}

		if env, ok := os.LookupEnv("LOTUS_WPOST_TASK"); ok {
			log.Debugf("env LOTUS_WPOST_TASK: %s", env)
		}
